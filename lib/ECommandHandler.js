...
import ECommand, { EMOJI_NO, makeError }        from './ECommand.js';
...
export default class ECommandHandler {
	...
	async handle(message) {
		...
		if (command.disabled) {
			return this.sendError(message, `${Formatters.inlineCode(command.name)} has been globally disabled`);
		}

		if (message.guild && this.client.provider.get(message.guild, 'disabledCommands', {})[command.name]) {
			return this.sendError(message, `${Formatters.inlineCode(command.name)} has been disabled in this guild`);
		}
		...
		if (message.guild) {
			...
			if (missingClientPerms.length) {
				return message.channel.send({
					embeds: [new MessageEmbed()
						.setColor('RED')
						.addField('I need the following permissions to execute this command', missingClientPerms.join('\n'))]
				});
			}
			...
			if (!isOwner) {
				...
				if (missingUserPerms.length) {
					return message.channel.send({
						embeds: [new MessageEmbed()
							.setColor('RED')
							.addField('You need the following permissions to execute this command', missingUserPerms.join('\n'))]
					});
				}
			}
		}
		...
		try {
			...
			if (typeof err !== 'string') {
				...
				return message.react(EMOJI_NO).catch();
			}

			const embed = makeError(err);
