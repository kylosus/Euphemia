import {
	Collection, inlineCode, EmbedBuilder, MessageType, PermissionFlagsBits, PermissionsBitField
} from 'discord.js';
...
export default class ECommandHandler {
	...
	async registerCommands(dir, moduleName) {
		...
		const slashCommands = parsedCommands.filter(c => c.slash).map(c => {
			...
			command.options = c.args.map(a => ({
				name:                     a.id,
				type:                     a.type.commandType,
				description:              a.description ?? 'idk',
				required:                 !a.optional,
				defaultMemberPermissions: a.userPermissions
			}));
		...
		this.client.on('ready', () => slashCommands.forEach(async c => {
			await this.client.application.commands.create(c, '292277485310312448');
