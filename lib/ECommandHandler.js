...
export default class ECommandHandler {
	...
	async handle(message) {
		...
		if (command.disabled) {
			return this.sendError(message, `${inlineCode(command.name)} has been globally disabled`);
		}

		if (message.guild && this.client.provider.get(message.guild, 'disabledCommands', {})[command.name]) {
			return this.sendError(message, `${inlineCode(command.name)} has been disabled in this guild`);
		}
		...
		if (message.guild) {
			...
			if (!isOwner) {
				const missingUserPerms = message.member.PermissionsBitField.missing(command.userPermissions);
